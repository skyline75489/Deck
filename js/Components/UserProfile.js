...
module.exports = React.createClass({
  ...
  componentDidMount: function() {
    var self = this;
    Api.getUserProfile(this.props.username, function(data) {
      self.setState({
        profileData: data,
        profileDataReady: true,
      });
    });
    Api.getUserRecentlyPushedRepo(this.props.username, 1, 5, function(data) {
      self.setState({
        repoListData: data,
  ...
  render: function() {
      ...
      infoList.push(
        <View key={Base.makeKey()} style={styles.itemWrapper}>
          <View style={styles.iconWrapper}>
            <Icon name="location" size={16} color="#666666" style={styles.icon}/>
          </View>
    ...
    if (data.email) {
      infoList.push(
        <View key={Base.makeKey()} style={styles.itemWrapper}>
          <View style={styles.iconWrapper}>
            <Icon name="mail" size={16} color="#666666" style={styles.icon}/>
          </View>
          <View style={styles.textWrapper}>
    ...
    if (data.blog) {
      infoList.push(
        <View key={Base.makeKey()} style={styles.itemWrapper}>
          <View style={styles.iconWrapper}>
            <Icon name="link" size={16} color="#666666" style={styles.icon}/>
          </View>
          <View style={styles.textWrapper}>
